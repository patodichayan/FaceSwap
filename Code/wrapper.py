...
from video import*
from PRnetSwap import prnetSwap
...
def main():
	...
	Parser.add_argument('--method',default = 'tps', help='Provide Method of image transformation: delaunay(deln), Thin Plate Spline(tps)')
	Parser.add_argument('--mode',default = 2, help='If swapping 1 image in a video, use 1, If swapping 2 faces in a single video, use 2')
	Parser.add_argument('--isDlib',default = "False", help='True if dlib should be used prediction of facial landmarks. False for using PrNet for the same.')
	...
	if mode == 1:
		...
		while (cap.isOpened()):
			...
			if ret == False:
				break
			if(count==0):
				showImage(landmarksImgTarget, "landmarks")
		...
		while (cap.isOpened()):
			...
			if method == 'tps':
				...
				_, result = prnetSwap(frame, frame)
			showImage(result, "swapped")
			cv2.imwrite("../Frames/Frame{}.png".format(count), result)
